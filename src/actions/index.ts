...
const ProfilePatchSchema: z.ZodType<ProfilePatch> = ProfileDtoSchema
  .partial()
  .merge(z.object({ UserId: z.string().regex(/^\d+$/, "UserId must be a string of digits") }));
...
export const server = {
  saveProfile: defineAction({
    ...
    handler: async (input, context) => {
      ...
      if (!user) {
        throw new ActionError({ code: "UNAUTHORIZED", message: "You must be signed in to update your profile." });
      }

      const authenticatedId = user.discordId || (user as unknown as { discordId?: string }).discordId;
      if (!authenticatedId) {
        throw new ActionError({ code: "UNAUTHORIZED", message: "Missing authenticated user id." });
      }

      if (input.UserId !== authenticatedId) {
        throw new ActionError({ code: "FORBIDDEN", message: "You are not allowed to modify this profile." });
      }
