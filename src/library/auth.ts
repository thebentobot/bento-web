...
export const createAuth = (d1: D1Database, request?: Request, env?: AuthEnv) => {
    ...
    const betterAuthUrl = (typeof import.meta !== "undefined" ? import.meta.env?.BETTER_AUTH_URL : undefined) ?? env?.BETTER_AUTH_URL
    const envOrigin = (() => {
        if (!betterAuthUrl) return undefined
        try { return new URL(betterAuthUrl).origin } catch { return undefined }
    })()
    const requestUrl = request ? new URL(request.url) : undefined
    const requestOrigin = requestUrl?.origin
    const host = requestUrl?.host || ""
    const isAstroDev = host.endsWith(":4321") || host === "localhost:4321" || host === "127.0.0.1:4321"
    ...
    const trustedOrigins = (() => {
        ...
        const csv = (typeof import.meta !== "undefined" ? import.meta.env?.BETTER_AUTH_TRUSTED_ORIGINS : undefined) ?? env?.BETTER_AUTH_TRUSTED_ORIGINS
        if (csv && typeof csv === "string") {
            for (const part of csv.split(/[,\s]+/)) {
                const p = part.trim()
                if (!p) continue
                try { set.add(new URL(p).origin) } catch (e) { /* ignore invalid URL in trusted origins */ void e }
            }
        }
    ...
    const DISCORD_CLIENT_ID = (typeof import.meta !== "undefined" ? import.meta.env?.DISCORD_CLIENT_ID : undefined) ?? env?.DISCORD_CLIENT_ID
    const DISCORD_CLIENT_SECRET = (typeof import.meta !== "undefined" ? import.meta.env?.DISCORD_CLIENT_SECRET : undefined) ?? env?.DISCORD_CLIENT_SECRET
