<script lang="ts">
  ...
  let { label = "Description", value = $bindable(""), maxLength = 500, placeholder = "Profile bio" } = $props();
  ...
  const counterClass = $derived(remaining < 20 ? "text-red-500" : remaining < 50 ? "text-amber-500" : "text-zinc-500");
</script>

<label class="grid gap-1">
  <span class="text-sm text-zinc-600 dark:text-zinc-400">{label} (Max length of {maxLength} characters)</span>
  <span class="rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 focus-within:ring-2 focus-within:ring-yellow-500 focus-within:ring-offset-2 focus-within:ring-offset-white dark:focus-within:ring-offset-zinc-900">
    <textarea
