...
<div class="grid gap-3 w-full">
  <ColorOpacityPicker label="Sidebar Colour" bind:color={sidebarColour} bind:opacity={sidebarOpacity} />
  ...
  <div class="grid sm:grid-cols-2 gap-3">
    <label class="grid gap-1">
      ...
      <input class={`input ${timezone && !timezoneValid ? "border-red-500 dark:border-red-600" : ""}`} placeholder="Europe/Copenhagen" bind:value={timezone} list="timezone-list" autocomplete="off" autocapitalize="off" spellcheck="false" inputmode="text" oninput={scheduleTimezoneDebounce} />
    ...
    <label class="grid gap-1">
      ...
      <span class="flex gap-2">
        <input class="input flex-1" type="date" value={birthdayInput} oninput={onBirthdayInput} />
        {#if birthday}
          <button class="btn-secondary" type="button" onclick={() => { birthday = null; }}>Remove</button>
        {/if}
      </span>
