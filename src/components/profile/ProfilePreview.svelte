...
<div class="relative w-full max-w-[600px] mx-auto aspect-[3/2]" bind:this={previewWrapper}>
  ...
  >
    ...
    <div
      ...
      onclick={(e) => { e.stopPropagation(); onOpen("description"); }}
    >
      <div class="absolute bottom-0 w-[300px] text-[20px] leading-tight">
        <p class="m-0" style={`color:${descriptionColourFull}`}>{profile.Description ?? "I am a happy user of Bento 🍱😄"}</p>
      </div>
    </div>
    ...
    <div
      ...
      onclick={(e) => { e.stopPropagation(); onOpen("sidebar"); }}
    >
      <!-- Avatar -->
      <div class="absolute" style="left:100px; transform: translate(-50%, 16px); width:96px; height:96px; border-radius:50%; overflow:hidden; z-index:2;">
        {#if avatarUrl}
      ...
      <div class="absolute" style="top:120px; width:200px; text-align:center;">
        <div class="font-['Urbanist']" style={`font-size:${usernameSize};color:${profile.UsernameColour ?? "#ffffff"}`}>{username}</div>
        <div class="font-['Urbanist'] text-[17px]" style={`color:${profile.DiscriminatorColour ?? "#9CA3AF"}`}>{discriminator ? `#${discriminator}` : ""}</div>
      </div>

      <!-- Sidebar list -->
      <ul class="list-none m-0 p-0 text-center absolute" style="top:170px; right:0; width:200px; line-height:1.1;">
        <li class="pt-[13px] h-auto" style={`color:${profile.SidebarItemServerColour ?? "#D3D3D3"}`}>
          <span class="text-[24px]" style={`color:${profile.SidebarValueServerColour ?? "#ffffff"}`}>Rank 0</span><br />Of 0 Users
        </li>
        <li class="pt-[13px] h-auto" style={`color:${profile.SidebarItemGlobalColour ?? "#D3D3D3"}`}>
          <span class="text-[24px]" style={`color:${profile.SidebarValueGlobalColour ?? "#ffffff"}`}>Rank 0</span><br />Of 0 Users
        </li>
        <li class="pt-[13px] h-auto" style={`color:${profile.SidebarItemBentoColour ?? "#D3D3D3"}`}>
          <span class="text-[24px]" style={`color:${profile.SidebarValueBentoColour ?? "#ffffff"}`}>0 🍱</span><br /> Rank 0/0 🍱 Users
        </li>
        <li class="pt-[13px] h-auto" style={`color:${profile.SidebarItemTimezoneColour ?? "#D3D3D3"}`}>
          <span class="text-[24px]" style="color:#ffffff">😎</span><br /> {tzDisplay} {birthdayDisplay}
        </li>
      </ul>
    </div>
    ...
    <div class="absolute w-[400px] h-[150px] flex flex-col gap-2" style={`left:0; top:250px; padding-top:${fmPaddingTop}; padding-left:20px;`}>
      {#if profile.LastfmBoard}
        <div
          ...
          onclick={(e) => { e.stopPropagation(); onOpen("lastfm"); }}
        >
          <div class="w-9 h-9 bg-zinc-700 rounded"></div>
          <div class="overflow-hidden">
            <div class="truncate" style={`color:${fmSongTextColourFull}; font-size:14px`}>Last.fm song title</div>
            <div class="truncate" style={`color:${fmArtistTextColourFull}; font-size:12px`}>Artist name</div>
          </div>
        </div>
      {/if}
      {#if profile.XpBoard}
        <div
          ...
          onclick={(e) => { e.stopPropagation(); onOpen("xp"); }}
        >
