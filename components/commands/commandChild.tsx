...
export default function CommandChild({ name, aliases, description, usage }: commandsInterface) {
	return (
		<li className="transition duration-300 ease-in-out bg-gray-800 items-center w-full my-4 px-0 lg:px-6 py-0 lg:py-6 p-4 rounded-lg group hover:bg-gray-700 shadow overflow-auto">
			<h1 id={name} className="text-2xl font-extrabold text-yellow-400 sm:text-xl text-center">
				<Link href={`#${name}`} className="text-yellow-400 hover:underline">
					{capitalize(name)}
				</Link>
			</h1>
			...
			<h1
				id={name}
				className="text-lg font-extrabold text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center"
			>
				How to use{" "}
				<a className="text-yellow-400 text-opacity-50 sm:text-xl group-hover:text-opacity-100">{capitalize(name)}</a>
			</h1>
			...
			{name !== "welcome" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Arguments is identical with the{" "}
						<Link href="#bye" className="hover:underline text-yellow-400 text-opacity-50 group-hover:text-opacity-100">
							Bye
						</Link>{" "}
						command arguments
					</p>
			...
			{name !== "horoscope" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						An example of when you have saved your horoscope and checks for today. You need to specify{" "}
						<code>tomorrow</code> or <code>yesterday</code> as an argument, but <code>today</code> can be omitted.
					</p>
					<div className="mt-2 opacity-50 group-hover:opacity-100">
						<Horoscope />
					</div>
			...
			{name !== "case" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Get a list of all kinds of punishments for a user. <br /> If you add <code>global</code> as the last
						argument you get all cases for the user, if they are on multiple Bento 🍱 servers.
					</p>
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						If a server has enabled privacy settings with the{" "}
						<Link
							href="#caseglobal"
							className="hover:underline text-yellow-400 text-opacity-50 group-hover:text-opacity-100"
						>
							Caseglobal
						</Link>{" "}
						command, both the server where the punishment happened and the reason why will be hidden.
					</p>
					<div className="mt-2 opacity-50 group-hover:opacity-100">
						<CaseUser />
					</div>
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Check individual cases by <strong>case number</strong>. <br /> Case numbers are listed when a punishment is
						handed out, and when checking cases for a user.
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Delete individual cases by <strong>case number</strong>. <br /> You need to confirm once before the delete
						happens, where it shows a preview of the case before you decide.
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Search for cases. <br /> You search by choosing a type of punishment, one of the potential columns (reason,
						notes, id of who banned e.g.), and at last the query you search with.
			...
			{name !== "mute" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						You can both mute for an indefinite time by using the mute command just like how you would use{" "}
						<Link href="#ban" className="hover:underline text-yellow-400 text-opacity-50 group-hover:text-opacity-100">
							Ban
						</Link>
						,{" "}
						<Link href="#kick" className="hover:underline text-yellow-400 text-opacity-50 group-hover:text-opacity-100">
							Kick
						</Link>{" "}
						and{" "}
						<Link
						...
						.
						<br /> But you can also specify a time where the user needs to be unmuted automatically, by writing a number
						and use the wished timeframe above.
			...
			{name !== "notification" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						Note - Bento 🍱 deletes your message every time you use a notifications command, so privacy of your
			...
			{name !== "sketch" ? null : (
				<div>
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						How to use some of the various arguments. <br /> A lot of the different layers has the same syntax (e.g.
						opacity and colour).
					...
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						For the sidebar you need to specify the row that you want to change colour of. <br />{" "}
						<code>?sketch sidebar opacity 50</code> and <code>?sketch sidebar colour #34D399</code> changes the
						background of the sidebar, not the text. You need to specify what text to change the colour of. <br />
						<br /> <code>?sketch sidebar rank</code> is for changing the colour of the rank text. The next argument{" "}
						<code>?sketch sidebar rank server</code> is where you add the row you want to change afterwards. The
						possible rank rows are either <code>server</code>, <code>global</code> or <code>bento</code>. <br /> Same
						syntax for changing the users text, where it is <code>?sketch sidebar users</code> and the possible rows{" "}
						<code>server</code>, <code>global</code>, <code>bento</code> or <code>timezone</code>.
					</p>
					<div className="mt-2 opacity-50 group-hover:opacity-100">
						<Sketch3 />
					</div>
					<p className="text-xl py-2 whitespace-pre-line text-gray-300 text-opacity-50 sm:text-xl group-hover:text-opacity-100 text-center max-w-xl mx-auto">
						For editing the xpboard, you can either choose to edit one specific element or two at the same time. <br />{" "}
						When using <code>?sketch xpboard text</code> it edits for both level texts, whereas{" "}
						<code>?sketch xpboard text1</code> only edits the level text to the left. <br /> When editing the colours of
						the XP progress bars, you need to specify the colour or opacity for all three colours. <br /> Check the
						examples below.
					</p>
					<div className="mt-2 opacity-50 group-hover:opacity-100">
						<Sketch4 />
